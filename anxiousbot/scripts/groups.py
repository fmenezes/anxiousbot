...
def _group(symbols_file_path, marketcap_file_path, out, json_out):
    ...
    for symbol in symbols:
        if ":" in symbol["symbol"]:
            print(
                f"{symbol['symbol']} does not contain format BASECOINT/QUOTECOIN",
                file=sys.stderr,
            )
            continue
        ...
        if len(symbol["exchanges"]) < 2:
            print(
                f"{symbol['symbol']} supports only one exchange: {symbol['exchanges'][0]}",
                file=sys.stderr,
            )
            continue
        ...
        if marketcap[base_coin].get("#", "") == "":
            print(
                f"{symbol['symbol']} does not have marketcap ranking", file=sys.stderr
            )
            continue
        rows.append(
            [
...
async def _run():
    ...
    if not os.path.exists(symbols_file_path):
        raise RuntimeError(
            f'file "{symbols_file_path}" not found, run "poetry run symbols"'
        )
    marketcap_file_path = "./data/marketcap.csv"
    if not os.path.exists(marketcap_file_path):
        raise RuntimeError(
            f'file "{marketcap_file_path}" not found, run "poetry run marketcap"'
        )
    _group(
        symbols_file_path, marketcap_file_path, groups_file_path, groups_json_file_path
    )
