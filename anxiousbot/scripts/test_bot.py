...
async def _echo(bot: Bot, update_id: int) -> int:
    ...
    for update in updates:
        ...
        if update.message and update.message.text:
            print(f"({update.effective_message.chat_id}) {update.effective_message.from_user.username}: {update.effective_message.text}")
...
async def _run():
    ...
    async with Bot(BOT_TOKEN) as bot:
        ...
        tasks = [asyncio.create_task(_listen(bot)), asyncio.create_task(_test(bot, BOT_CHAT_ID))]
